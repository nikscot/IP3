...
***Test Case***
CSV to JSON to CSV Yields Original   
	${contents}=     Get File                src/test/resources/grades.csv
	${json}=         Convert To Json         ${contents}
	${csv}=          Convert To Csv          ${json}
	Should Be Equal  ${contents}             ${csv}
JSON to CSV to JSON Yields Original
	${contents}=     Get File                src/test/resources/grades.json
